...
| PC (Screen Reader) | JAWS / NVDA / Windows Narrator | [Menu navigation announcements] | Not Started | Requires UI elements to expose accessible names and roles via platform UI layer. Godot 4.5+ AccessKit integration covers this for supported control types. Unity 2023+ supports UI Toolkit accessibility. Ebitengine requires custom accessibility implementation for Go apps. Verify against engine-reference/[engine]/ docs. |
